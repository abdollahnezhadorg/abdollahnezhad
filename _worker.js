    ...
    warpEndpoints.split(',').forEach( (endpoint, index) => {
        ...
        if (client === 'singbox' || client === 'hiddify') {
            let singboxOutbound = buildSingboxWarpOutbound(
                client === 'hiddify' ? `POORIA Warp Pro ${index + 1} 🔴` : `POORIA Warp ${index + 1} 🔴`, 
                warpIPv6, 
                privateKey, 
                publicKey, 
                endpoint,
...
async function getXrayWarpConfigs (env, client) {
    ...
    xrayWarpBestPing.remarks = client === 'nikang' ? 'POORIA Warp Pro Best Ping 🚀' : 'POORIA Warp Best Ping 🚀';
    ...
    xrayWoWOutbounds.forEach((outbound, index) => {
        if (outbound.tag === 'chain') {
            ...
            xrayWoWConfig.remarks = client === 'nikang' ? `POORIA WoW Pro ${proxyIndex} 🟡` : `POORIA WoW ${proxyIndex} 🟡`;
